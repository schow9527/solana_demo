...
/*在 Anchor 框架里，每个指令（Instruction）都对应一个 上下文（Context），它包含两个部分：

Accounts：指令执行所需的所有链上账户

Instruction Data：指令传入的参数（比如你要初始化 store 的 admin 地址）

Anchor 用 上下文结构体（Context Struct） 来把账户和参数绑定在一起，让你的指令函数能够安全访问这些账户。

*/
#[derive(Accounts)]
#[instruction(params: InitStoreParams)]
pub struct InitStore<'info> {//'info 是一个 生命周期参数（lifetime）,这个结构体里的引用（accounts）至少要活得和 'info 一样长
    #[account(mut)]
...
impl InitStore<'_> {
    pub fn apply(ctx: &mut Context<InitStore>, params: &InitStoreParams) -> Result<()> {
        ...
        let register_params = RegisterOAppParams { delegate: ctx.accounts.store.admin };//作为接收方需要的
