...
// -----------------------------------------------------------------------------
// Payment message codec
// 使用 AnchorSerialize/AnchorDeserialize 直接进行 Borsh 编解码
// -----------------------------------------------------------------------------

#[derive(Clone, Debug, PartialEq, AnchorSerialize, AnchorDeserialize)]
pub struct AddressType {
    pub chain_id: u16,      // 源链/目标链的 chain id（应用自定义）
    pub addr: Vec<u8>,      // 原始地址字节，EVM 为 20 bytes，Solana 为 32 bytes
}
...
pub struct PaymentMsg {
    ...
    pub token: AddressType,
    pub amount: u64,
}

pub fn encode_payment(msg: &PaymentMsg) -> Vec<u8> {
    msg.try_to_vec().expect("borsh encode")
}

pub fn decode_payment(buf: &[u8]) -> std::result::Result<PaymentMsg, MsgCodecError> {
    PaymentMsg::try_from_slice(buf).map_err(|_| MsgCodecError::InvalidLength)
}
